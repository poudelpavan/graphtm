...
static int all_commits = 0, all_aborts = 0;
...
int_stm_exit_thread(stm_tx_t *tx)
{
  ...
  all_commits += tx->stat_commits;
  all_aborts += tx->stat_aborts;
  PRINT_DEBUG("==> stm_exit_thread(%p[%lu-%lu])\n", tx, (unsigned long)tx->start, (unsigned long)tx->end);
  double avg_aborts = .0;
  if (tx->stat_commits)
    avg_aborts = (double)tx->stat_aborts / tx->stat_commits;
  // printf("Thread %p => RW set size:%6u | commits:%6u | aborts:%6u | avg_aborts:%8.4f | max_retries:%6u \n", (void *)pthread_self(), RW_SET_SIZE, tx->stat_commits, tx->stat_aborts, avg_aborts, tx->stat_retries_max);
  // printf("Total commits: %8d | Total aborts: %8d \n", all_commits, all_aborts);
...
static volatile int total_txss = 0;
...
int_stm_start(stm_tx_t *tx, stm_tx_attr_t attr)
{
  ...
  for(int x = 0; x < sizeof(tx->r_set.entries); x++){
    if(tx->r_set.entries[x].version != NULL){
      printf("rs-%d-%p,",total_txss,tx->r_set.entries[x]);
    }
  }
  // printf("\nWrite set: size = %d",sizeof(tx->w_set.entries));
  // printf("\n");
  for(int x = 0; x < sizeof(tx->w_set.entries); x++){
    if(tx->w_set.entries[x].addr != NULL){
      printf("ws-%d-%p,",total_txss,tx->w_set.entries[x].version);
    }
  }
  total_txss++;
  // printf("\n %d",total_txss);
